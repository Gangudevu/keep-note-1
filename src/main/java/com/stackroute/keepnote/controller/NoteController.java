...
public class NoteController {
	...
	Note note = obj.getBean("note",Note.class);
	NoteRepository noteRepo = obj.getBean("noteRepository", NoteRepository.class);
	...
	*/
	...
	public String addNote(ModelMap model, @RequestParam("noteId") Integer noteId,
		...
		if(noteId == null || noteTitle.isEmpty() || noteContent.isEmpty() || noteStatus.isEmpty()) {
			model.addAttribute("error","Fill all the required fields");
		}
		else if(noteRepo.exists(noteId)){
			model.addAttribute("error","Noteid already exists");
		}
		else {
			Note n = obj.getBean("note",Note.class);
			n.setNoteId(noteId);
		...
		model.addAttribute("savedNotes",noteRepo.getList());
		return "index";
	...
	*/
	...
	public String deleteNote(ModelMap model, @RequestParam("noteId") Integer noteId) {
